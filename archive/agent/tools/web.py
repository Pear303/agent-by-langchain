"""网页抓取工具：web_fetch。

功能：
- 获取指定 URL 的网页内容
- 支持两种提取模式：text（纯文本，默认）和 raw（原始 HTML）
- 可限制最大返回字符数
- 只读操作（read_only=True，可并发执行）
"""
from __future__ import annotations
import re
import urllib.request
from html.parser import HTMLParser

from .base import Tool
from .schema import StringSchema, IntegerSchema


class _TextExtractor(HTMLParser):
    """HTML 文本提取器。
    
    从 HTML 中提取纯文本，跳过 script 和 style 标签的内容，
    在块级元素后添加换行符以保持可读性。
    """
    
    def __init__(self):
        super().__init__()
        self._parts: list[str] = []  # 文本片段列表
        self._skip = False  # 是否跳过当前标签内容

    def handle_starttag(self, tag, attrs):
        """处理开始标签。
        
        Args:
            tag: 标签名
            attrs: 属性列表
        """
        if tag in ("script", "style"):
            self._skip = True  # 跳过脚本和样式内容

    def handle_endtag(self, tag):
        """处理结束标签。
        
        Args:
            tag: 标签名
        """
        if tag in ("script", "style"):
            self._skip = False
        # 在块级元素后添加换行符
        if tag in ("p", "br", "div", "li", "tr", "h1", "h2", "h3", "h4"):
            self._parts.append("\n")

    def handle_data(self, data):
        """处理文本数据。
        
        Args:
            data: 文本内容
        """
        if not self._skip:
            self._parts.append(data)

    def get_text(self) -> str:
        """获取提取的纯文本。
        
        Returns:
            清理后的文本（合并多余换行）
        """
        return re.sub(r"\n{3,}", "\n\n", "".join(self._parts)).strip()


def _fetch(url: str, extract_mode: str = "text", max_chars: int = 8000) -> str:
    """抓取网页内容。
    
    Args:
        url: 要访问的 URL
        extract_mode: 提取模式（text 或 raw）
        max_chars: 最大返回字符数
        
    Returns:
        提取的文本内容或错误消息
    """
    req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0"})
    try:
        with urllib.request.urlopen(req, timeout=10) as resp:
            raw = resp.read().decode("utf-8", errors="replace")
    except Exception as e:
        return f"Error fetching {url}: {e}"

    # 根据模式提取内容
    if extract_mode == "text":
        parser = _TextExtractor()
        parser.feed(raw)
        text = parser.get_text()
    else:
        text = raw

    return text[:max_chars]


class WebFetch(Tool):
    """网页抓取工具。
    
    获取指定 URL 的网页内容，支持文本提取模式。
    由于是只读操作，标记为 read_only（可并发执行）。
    """
    
    name = "web_fetch"
    description = "获取指定 URL 的网页内容，支持文本提取模式"
    read_only = True  # 只读操作，可并发执行

    @property
    def parameters(self) -> dict:
        """工具参数 JSON Schema 定义。
        
        Returns:
            包含 url、extract_mode、max_chars 三个参数的 Schema
        """
        return {
            "type": "object",
            "properties": {
                "url":          StringSchema("要访问的完整 URL").to_json_schema(),
                "extract_mode": StringSchema(
                    "提取模式：text（纯文本，默认）或 raw（原始 HTML）",
                    enum=["text", "raw"],
                ).to_json_schema(),
                "max_chars":    IntegerSchema(
                    "最大返回字符数，默认 8000", minimum=1,
                ).to_json_schema(),
            },
            "required": ["url"],
        }

    def execute(self, url: str, extract_mode: str = "text", max_chars: int = 8000) -> str:
        """执行网页抓取。
        
        Args:
            url: 要访问的 URL
            extract_mode: 提取模式（text 或 raw）
            max_chars: 最大返回字符数
            
        Returns:
            提取的网页内容
        """
        print(f"[网页获取]: {url}")
        return _fetch(url, extract_mode, max_chars)